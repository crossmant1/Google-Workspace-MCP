...
3. Search for and enable all the APIs you will need, e.g., Gmail API, Calendar API.

4. From the sidebar, click **OAuth Consent Screen**, then from the new sidebar, click **Audience**.
...
10. Go to the server's environment variables or .env file. The server should need the Client ID and Client secret to function, so you can add these.

11. Find the server's endpoint URI and add it to **'Authorized Access URIs.'** Also add the server's endpoint with the authentication endpoint. E.g. `https://the-service-on-something.com/oauth2callback`. Additionally, add <https://flyergpt.udayton.edu>.
...
10. At the bottom, under **'Exceptions,'** select **'Allow Azure services and resources to access this server.'**
...
1. Ensure a Docker image has been created from the server.
...
4. Enter your Docker image URL with the following syntax: `docker.io/your-docker-username/your-online-image-name:latest`
